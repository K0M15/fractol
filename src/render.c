...
/*   Updated: 2024/12/24 14:05:38 by afelger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	render_square(t_renderstate *renders, t_renderparam *para, t_appstate *state)
{
	renders->j = 0;
	while (renders->j < para->fill && renders->y + renders->j < HEIGHT)
	{
		...
		{
			mlx_put_pixel(state->image, renders->x + renders->i, renders->y + renders->j, MAPS(renders->col, state->depth));
			renders->i++;
		}
		renders->j++;
	}
...
void	render(t_appstate *state, t_vec4 map, t_renderparam para, int (* fract)(t_vec2, t_ldb, t_ldb, int))
{
	...
	while(renders.y < HEIGHT)
	{
		...
		{
			map_pixel_screen(&renders.buffer, renders.x, renders.y, map, renders.screen);
			renders.col = fract(renders.buffer, state->fractParam.x, state->fractParam.y, state->depth);
